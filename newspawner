...
spawnAllButton.Text = "Spawn All Versions"
...
-- Define all possible version combinations
local allVersions = {
    -- Regular versions
    {F = false, R = false, N = false, M = false}, -- Regular
    {F = true,  R = false, N = false, M = false}, -- Fly only
    {F = false, R = true,  N = false, M = false}, -- Ride only
    {F = true,  R = true,  N = false, M = false}, -- Fly & Ride
    
    -- Neon versions
    {F = false, R = false, N = true,  M = false}, -- Neon only
    {F = true,  R = false, N = true,  M = false}, -- Neon Fly
    {F = false, R = true,  N = true,  M = false}, -- Neon Ride
    {F = true,  R = true,  N = true,  M = false}, -- Neon Fly & Ride
    
    -- Mega Neon versions
    {F = false, R = false, N = false, M = true},  -- Mega only
    {F = true,  R = false, N = false, M = true},  -- Mega Fly
    {F = false, R = true,  N = false, M = true},  -- Mega Ride
    {F = true,  R = true,  N = false, M = true},  -- Mega Fly & Ride
}

-- Function to get version name
local function getVersionName(flags)
    if flags.M then
        local name = "Mega Neon"
        if flags.F or flags.R then
            name = name .. " ("
            if flags.F then name = name .. "Fly" end
            if flags.F and flags.R then name = name .. " & " end
            if flags.R then name = name .. "Ride" end
            name = name .. ")"
        end
        return name
    elseif flags.N then
...
spawnAllButton.MouseButton1Click:Connect(function()
    local currentTime = os.clock()
    local extendDuration = 5.0

    local isExtension = currentTime < allActiveAnimation.endTime

    if isExtension then
        allActiveAnimation.intensity = math.min(allActiveAnimation.intensity + 0.3, 2.0)
        extendDuration = 5.0
    else
    ...
    -- Show notification that spawning has started
    game.StarterGui:SetCore("SendNotification", {
        Title = "Mass Spawning...",
        Text = "Spawning " .. #highTierPets .. " pets × " .. #allVersions .. " versions",
        Duration = 4
    })
    
    local totalSuccess = 0
    local totalPets = #highTierPets * #allVersions
    local completedPets = 0
    
    -- Spawn each pet with all versions
    for _, petName in ipairs(highTierPets) do
        for versionIndex, versionFlags in ipairs(allVersions) do
            if _G.spawn_pet(petName, versionFlags) then
                totalSuccess = totalSuccess + 1
                completedPets = completedPets + 1
                
                -- Update progress every 10 pets
                if completedPets % 10 == 0 then
                    local progress = math.floor((completedPets / totalPets) * 100)
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "Progress...",
                        Text = progress .. "% complete (" .. completedPets .. "/" .. totalPets .. ")",
                        Duration = 1
                    })
                end
            end
            wait(0.05) -- Small delay between spawns
        end
    end
    ...
    if totalSuccess > 0 then
        ...
        local notificationText = "Successfully spawned " .. totalSuccess .. " pets!"
        notificationText = notificationText .. "\n(" .. #highTierPets .. " pets × " .. #allVersions .. " versions)"
        ...
        print("=== MASS SPAWN SUMMARY ===")
        print("Pets: " .. #highTierPets)
        print("Versions per pet: " .. #allVersions)
        print("Total attempted: " .. totalPets)
        print("Successfully spawned: " .. totalSuccess)
        print("Failed: " .. (totalPets - totalSuccess))
        print("========================")
    else
